...
TODO: Dans le chapitre précédent, nous avons vu comment allouer dynamiquement des ressources matérielles et ordonnancer les requêtes utilisateur dans une plateforme serverless pour des applications simples, composées d'une unique fonction. Un défi majeur dans le paradigme serverless, lorsque l'on souhaite garantir la qualité de service pour les utilisateurs, survient lors de la composition des fonctions. Ce mécanisme consiste à chaîner des appels de fonctions pour réaliser des tâches complexes, qui induisent des dépendances temporelles et de données. Une chaîne de fonctions est appelée \textit{application} (parfois \textit{workflow}~\cite{burckhardtNetheriteEfficientExecution}). Au cours de l'exécution d'une telle application, les instances des fonctions qui la composent peuvent être allouées dynamiquement sur tout nœud de l'infrastructure par la plateforme serverless, provoquant des démarrages à froid et des délais de communication qui s'additionnent tout au long de la chaîne, dégradant la qualité de service requise par les différents utilisateurs.
...
\subsection{Détection d'instrusion à l'edge}
...
TODO: La figure~\ref{figure:herocache-function-cache} illustre le cycle de vie d'une requête dans une plateforme serverless.
...
TODO: comme c'est le cas pour les deux fonctions déployées sur les nœuds 2 et 3 dans la figure~\ref{figure:herocache-function-communications}...
...
% TODO: Concernant la caractérisation du stockage distant, nous avons utilisé des mesures issues de l'état de l'art.
